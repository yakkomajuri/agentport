...
@pytest.mark.anyio
async def test_mcp_allow_rule_records_matched_rule_on_executed_log(mcp_env):
    """An allow rule should stamp matched_rule_id on the executed LogEntry."""
    import json as _json
    ...
    with Session(engine) as session:
        ...
        session.add(
            ToolExecutionRuleCondition(
                ...
                param_path="content",
                operator="contains",
                values_json=_json.dumps(["ok"]),
            )
        )
        session.commit()
        rule_id = rule.id

    fake = AsyncMock(return_value={"content": [{"type": "text", "text": "done"}]})
    with patch("agent_port.mcp.server.mcp_client.call_tool", side_effect=fake):
        await execute_upstream_tool("posthog", "create_annotation", {"content": "all ok"})
